...
export default function SettingsOverlay({ isOpen, onClose }) {
  ...
  // The overlay is position:fixed so it lives outside #app's cascade.
  // We must inject the current font/size/spacing directly onto it.
  const panelStyle = {
    fontFamily:    activeMode?.ff ?? "'Segoe UI',system-ui,sans-serif",
    fontSize,
    letterSpacing: activeMode?.ls ?? '0em',
    lineHeight:    activeMode?.lh ?? '1.6',
  }
