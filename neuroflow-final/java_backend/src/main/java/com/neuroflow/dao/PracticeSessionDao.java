...
public class PracticeSessionDao {
    ...
    public PracticeSessionDao(DatabaseManager db) { this.db = db; }
    ...
    public int countTodayAttempts(int studentId) {
        try (PreparedStatement ps = conn().prepareStatement(
                "SELECT COALESCE(SUM(attempts),0) FROM practice_sessions " +
                "WHERE student_id=? AND date(created_at) = date('now')")) {
            ps.setInt(1, studentId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return rs.getInt(1);
    ...
    /** Returns array[7] where index 0=Sunday ... 6=Saturday, value = session count */
    ...
    /** Returns the single most-recent session for this student, or null */
    public PracticeSession findLatest(int studentId) {
        try (PreparedStatement ps = conn().prepareStatement(
                "SELECT * FROM practice_sessions WHERE student_id=? ORDER BY id DESC LIMIT 1")) {
            ps.setInt(1, studentId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return map(rs);
        } catch (SQLException e) {
            System.err.println("[PracticeSessionDao] findLatest error: " + e.getMessage());
        }
        return null;
    }
}
