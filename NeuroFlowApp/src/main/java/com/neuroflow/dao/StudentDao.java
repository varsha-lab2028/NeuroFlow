...
public class StudentDao {
    ...
    public int insert(Student s) {
        try (PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO students(user_id, parent_id, educator_id, name, initials, " +
                        "streak_days, current_letter, weekly_progress, primary_issue, trend) " +
                        "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                Statement.RETURN_GENERATED_KEYS)) {
            ps.setInt(1, s.getUserId());
            ps.setInt(2, s.getParentId());
            ps.setInt(3, s.getEducatorId());
            ps.setString(4, s.getName());
            ps.setString(5, s.getInitials());
            ps.setInt(6, s.getStreakDays());
            ps.setString(7, s.getCurrentLetter());
            ps.setInt(8, s.getWeeklyProgress());
            ps.setString(9, s.getPrimaryIssue());
            ps.setString(10, s.getTrend());
            ps.executeUpdate();
